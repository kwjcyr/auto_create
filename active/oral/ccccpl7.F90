...
!   !LOCAL VARIABLES
    type(mct_gsMap), pointer :: gsMap_{c}
    type(mct_gGrid), pointer :: dom_{c}
    integer (kind(1)) :: nThreads
    integer (kind(1)) :: {CCC}ID, lsize
    real (r8) ::  precadj
  ...
  call seq_infodata_PutData( infodata, {ccc}_nx=(imt_global-2), {ccc}_ny=jmt_global)
  call seq_infodata_PutData(infodata, &
!{list} key="ccc_list"
                                {ccc_list}_prognostic=.true., &
                                &)
